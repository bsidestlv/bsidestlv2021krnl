...
static int ip6mr_sk_init(struct mr_table *mrt, struct sock *sk)
{
	...
	rtnl_lock();
	write_lock_bh(&mrt_lock);
  printk("rtnl_dereference(mrt->mroute_sk) = %x\n", rtnl_dereference(mrt->mroute_sk));
	if (rtnl_dereference(mrt->mroute_sk)) {
		err = -EADDRINUSE;
	} else {
		rcu_assign_pointer(mrt->mroute_sk, sk);
		sock_set_flag(sk, SOCK_RCU_FREE); // set the socket free to be RCU related.
			  ...
			  int __user *optlen)
{
	...
	mrt = ip6mr_get_table(net, raw6_sk(sk)->ip6mr_table ? : RT6_TABLE_DFLT); // enum is 254
	if (!mrt)
		return -ENOENT;
